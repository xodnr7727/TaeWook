...
namespace MyArray {
	...
	public:
		...
		// address를 초기화 하는 함수이다. 재귀 호출로 구성되어 있음.
		void initialize_address(Address* current) {
			if (!current) return;
			if (current->level == dim - 1) { // 종료 조건
		...
		void delete_address(Address* current) {
			if (!current) return;
			for (int i = 0; i != current->level < dim - 1 && i < size[current->level]; i++) {
	...
	public:
		Int(int index, int _level = 0, void* _data = NULL, Array* _array = NULL) // 디폴트 인자(default argument) 함수에 값을 전달해주지 않는다면 기본으로 인자에 이 값들이 들어감.
			...
			if (level == array->dim) {
				...
				// 그렇지 않을 경우로 data에 그냥 다음 addr을 넣어준다.
				data = static_cast<void*>(
