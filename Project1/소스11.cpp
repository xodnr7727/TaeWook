...
int main() {
	...
	for (int total = 1; total <= 1000000; total *= 10) {
		...
		ch::time_point< ch::high_resolution_clock> start =
			ch::high_resolution_clock::now(); //현재의 time_point 를 리턴하는 static 함수 now
		...
		cout << setw(7) << total
			<< "개 난수 생성시 걸리는 시간 : " << duration_cast<ch::microseconds>(diff).count() << "us"
			<< endl;
	}
}*/
...
int main() {
	...
	//random_device 를 통해 난수 생성 엔진을 초기화한다
	mt19937 gen(rd());
	normal_distribution<double> dist(0, 1); // 정규 분포 정의
