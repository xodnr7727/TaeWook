...
void AProjectNo1Character::OnNeckSkillPressed()
{
	...
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), ALichEnemy::StaticClass(), OverlappingActors);
	...
	if (CanNeckSkill() && HasEnoughSkillStamina())
	{
		...
		{
			// 몬스터 클래스로 캐스트하여 유효한 몬스터인지 확인

			if (LichEnemy && LichEnemy->Alive()) // IsAlive() 함수는 몬스터의 상태를 확인하는 함수입니다.
			{
				// 포효 범위 내에 있는 몬스터를 기절 상태로 변경
				FVector PlayerLocation = GetActorLocation();
				FVector MonsterLocation = LichEnemy->GetActorLocation();
				float DistanceToMonster = FVector::Distance(PlayerLocation, MonsterLocation);

				if (DistanceToMonster <= RoarRadius)
				{
					LichEnemy->Stun(); // 몬스터를 기절 상태로 변경하는 함수 (Stun 함수를 몬스터 클래스에 추가해야 함)
				}
			}
		}
	}
...
{
if (IsOccupied() && !HasEnoughStamina() && IsArm()) return;
	PlayDiveMontage();
